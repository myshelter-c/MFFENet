...
class ASPP_another(nn.Module):
    def __init__(self, in_channel):
        depth = in_channel
        super(ASPP_another,self).__init__()
...
class Attention_Module(nn.Module):
    def __init__(self, high_feature_channel, low_feature_channels, output_channel = None):
        ...
        self.relu = nn.ReLU(inplace = True)
        self.Conv1x1 = Conv1x1(low_feature_channels, low_feature_channels)
    def forward(self, high_features, low_features, lap):
        low_features = self.Conv1x1(torch.cat(low_features, 1))
        features = [upsample(high_features)]
        features += low_features.unsqueeze(0)
        features += lap.unsqueeze(0)
...
class High_Frequency_Upsample(nn.Module):
    def __init__(self, upscale_factor):
        super(High_Frequency_Upsample, self).__init__()
...
class Consist(nn.Module):
    def __init__(self, input_clannels, output_clannels):
        super(Consist, self).__init__()
        self.input_c = input_clannels
        self.output_c = output_clannels
        self.conv1x1 = Conv1x1(self.input_c, self.output_c)
        self.relu = nn.ReLU(inplace = True)
    
    def forward(self, x, y):
        y = upsample(y)
        features = torch.cat((x, y), dim=1)
        return self.relu(self.conv1x1(features))

class Consist_another(nn.Module):
    def __init__(self, input_clannels, output_clannels):
        super(Consist_another, self).__init__()
        self.input_c = input_clannels
        self.output_c = output_clannels
        self.conv1x1 = Conv1x1(self.input_c, self.output_c)
        self.relu = nn.ReLU(inplace = True)
    
    def forward(self, x, y, lap):
        y = upsample(y)
        features = torch.cat((x, y, lap), dim=1)
        return self.relu(self.conv1x1(features))


class Consist_another2(nn.Module):
    def __init__(self, input_clannels, output_clannels):
        super(Consist_another2, self).__init__()
        self.input_c = input_clannels
        self.output_c = output_clannels
        self.conv1x1 = Conv1x1(self.input_c, self.output_c)
        self.conv1x1_lap = Conv1x1(3, 3)
        self.relu = nn.ReLU(inplace = True)
    
    def forward(self, x, y, lap):
        lap = self.conv1x1_lap(lap)
        y = upsample(y)
        features = torch.cat((x, y, lap), dim=1)
        return self.relu(self.conv1x1(features))
