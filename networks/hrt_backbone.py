...
model_urls = {
    'hrt_small': './models/hrt_small_imagenet_pretrained_top1_812.pth',
    'hrt_tiny': './models/hrt_tiny_imagenet_pretrained_top1_786.pth'
}
...
class HighResolutionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        list32.append(y_list[0])
        list64.append(y_list[1])
        x_list = []
        ...
        list64.append(y_list[1])
        list128.append(y_list[2])
        mixed_features = [list32] + [list64] + [list128] + [y_list[3]]
        return features + mixed_features


class HRTBackbone(object):
    def __init__(self, network, pretrained):
    ...
    def __call__(self):
        ...
        if arch in [
            ...
            pretrained_url=""
            if arch == "hrt_small":
                pretrained_url = model_urls["hrt_small"]
            elif arch == "hrt_tiny":
                pretrained_url = model_urls["hrt_tiny"]
            else:
                pretrained_url = model_urls["hrt_base"]
...
if __name__ == "__main__":
    net = HRTBackbone("hrt_tiny", pretrained=True)()
    data = torch.zeros(8,3,192,640)
    para_sum = sum(p.numel() for p in net.parameters())
