...
class Trainer:
    def __init__(self, options):
        ...
        self.models["encoder"] = networks.HRTBackbone("hrt_tiny", pretrained=True)()
        self.models["encoder"].num_ch_enc = [ 64, 18, 36, 72, 144 ]

        # self.models["encoder"] = networks.HRTBackbone("hrt_small", pretrained=True)()
        # self.models["encoder"].num_ch_enc = [ 64, 32, 64, 128, 256 ]
        ...
        if num_gpus > 1:
            self.models["encoder"] = nn.parallel.DistributedDataParallel(self.models["encoder"], device_ids=[self.opt.local_rank],
                                                    output_device=self.opt.local_rank, find_unused_parameters=True)
            self.models["depth"] = nn.parallel.DistributedDataParallel(self.models["depth"], device_ids=[self.opt.local_rank],
                                                    output_device=self.opt.local_rank, find_unused_parameters=True)
            self.models["pose_encoder"] = nn.parallel.DistributedDataParallel(self.models["pose_encoder"], device_ids=[self.opt.local_rank],
                                                    output_device=self.opt.local_rank, find_unused_parameters=True)
            self.models["pose"] = nn.parallel.DistributedDataParallel(self.models["pose"], device_ids=[self.opt.local_rank],
                                                    output_device=self.opt.local_rank, find_unused_parameters=True)
        ...
        self.train_loader_k = DataLoader(
            train_dataset_k, self.opt.batch_size, shuffle=(self.train_sampler is None),
            num_workers=self.opt.num_workers, pin_memory=True, sampler=self.train_sampler, drop_last=True)
        ...
        self.val_loader = DataLoader(
            val_dataset, self.opt.batch_size, shuffle=(self.train_sampler is None),
            num_workers=self.opt.num_workers, pin_memory=True, sampler=self.val_sampler, drop_last=True)
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            outputs = self.models["depth"](inputs["color_aug", 0, 0], features)
    ...
    def compute_edge_loss_log(self, pred, target):


        c = 0.2 * torch.max(torch.abs(pred - target))

        abs_error = torch.abs(pred - target)
        L2 = (torch.abs(pred - target)**2 - c**2) / (2 * c)

        berHu_loss = torch.where(abs_error <= c, abs_error, L2)

        berHu_loss = berHu_loss.mean(1, True)
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            for frame_id in self.opt.frame_ids[1:]:
                ...
                edge_losses.append(self.compute_edge_loss_log(pred, target))

            relative_losses.append(self.compute_reprojection_loss(outputs[("color", -1, scale)], outputs[("color", 1, scale)]))
            
            reprojection_losses = torch.cat(reprojection_losses, 1)
            edge_losses = torch.cat(edge_losses, 1)
            relative_losses = torch.cat(relative_losses, 1)

            if not self.opt.disable_automasking:
                #doing this 
                identity_reprojection_losses = []
                identity_relative_losses = []
                for frame_id in self.opt.frame_ids[1:]:
                    pred = inputs[("color", frame_id, source_scale)]
                    identity_reprojection_losses.append(
                        self.compute_reprojection_loss(pred, target))
                identity_relative_losses.append(self.compute_reprojection_loss(inputs[("color", -1, source_scale)], inputs[("color", 1, source_scale)]))

                identity_reprojection_losses = torch.cat(identity_reprojection_losses, 1)
                identity_relative_losses = torch.cat(identity_relative_losses, 1)
                if self.opt.avg_reprojection:
                    identity_reprojection_loss = identity_reprojection_losses.mean(1, keepdim=True)
                else:
                    # save both images, and do min all at once below
                    identity_reprojection_loss = identity_reprojection_losses
                    identity_relative_loss = identity_relative_losses
            ...
            if not self.opt.disable_automasking:
                ...
                if torch.cuda.is_available():
                    ...
                    identity_relative_loss += torch.randn(identity_relative_loss.shape).cuda() * 0.00001
                else:
                    identity_reprojection_loss += torch.randn(identity_reprojection_loss.shape).cpu() * 0.00001
                    identity_relative_loss += torch.randn(identity_relative_loss.shape).cpu() * 0.00001
                combined = torch.cat((identity_reprojection_loss, reprojection_loss), dim=1)
                combined_relative = torch.cat((identity_relative_loss, relative_loss), dim=1)
                combined_edge = torch.cat((identity_reprojection_loss, edge_loss), dim=1)
            else:
            ...
            if combined.shape[1] == 1:
                ...
                to_optimis_relative, idxs_relative = torch.min(combined_relative, dim=1)
                to_optimise_edge, idxs_edge = torch.min(combined_edge, dim=1)
                #edge Filter
                another_filter = torch.logical_or(idxs_edge == 2, idxs_edge == 3)
                new_filter = torch.logical_and(another_filter, edge.squeeze(1))
                valid_edge_points = [torch.sum(i) for i in new_filter]
                to_optimise_edge *= new_filter
                edge_loss = torch.stack([torch.sum(to_optimise_edge) / valid_edge_points[i] for i in range(to_optimise_edge.shape[0])])
            if not self.opt.disable_automasking:
            ...
            loss += to_optimis_relative.mean() * self.opt.disparity_relative
            loss += self.opt.disparity_edge * edge_loss.mean()
